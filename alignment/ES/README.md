...
| File | Description |
|------|-------------|
| `es_fine_tuning_alignment.py` | Main ES training script |
| `reconstruct_checkpoint.py` | Reconstruct checkpoints from replay logs |
| `verify_reconstruction_vllm.py` | Verify checkpoint reconstruction |
| `run.sh` | SLURM launcher for training |
| `eval.sh` | SLURM launcher for evaluation |
| `test_replay_log.sh` | Test replay log checkpoint saving |
...
# 2. Update SCORER_HOST in alignment/ES/run.sh
