...
def _chat_complete(url: str, api_key: str, model: str, temperature: float, timeout_s: int, messages: List[dict], max_retries: int = 3) -> dict:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                continue
            return {"error": "timeout", "body": "Request timed out"}
        except Exception as e:
            return {"error": "exception", "body": str(e)}
    return {"error": "max_retries", "body": "Exceeded max retries"}
...
def judge_pairs(
    ...
    """
    Run pairwise judgments using multiple API keys for higher throughput.
    
    api_keys: List of API keys to round-robin across (each org has separate rate limits).
    """
    rng = random.Random(seed)
    ...
    with cf.ThreadPoolExecutor(max_workers=max_workers) as ex:
        fut_to_job = {}
        for idx, (q, dim, m_a, m_b, ans_a, ans_b, msgs, order) in enumerate(jobs):
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--api-key", type=str, default=None, help="Comma-separated API keys for parallel orgs, or set OPENAI_API_KEY/OPENAI_API_KEY_2")
...
def main() -> None:
    ...
    # Support multiple API keys for higher throughput (comma-separated or env vars)
    api_keys: List[str] = []
    if args.api_key:
        # Allow comma-separated keys via CLI
        api_keys = [k.strip() for k in args.api_key.split(",") if k.strip()]
    else:
        # Check environment variables: OPENAI_API_KEY, OPENAI_API_KEY_2, etc.
    ...
    print(f"Using {len(api_keys)} API key(s) for parallel requests")
    ...
    # Scale concurrency by number of API keys (each org has separate rate limits)
    effective_concurrency = max(1, args.concurrency) * len(api_keys)
    print(f"Effective concurrency: {effective_concurrency} workers ({args.concurrency} per key × {len(api_keys)} keys)")
